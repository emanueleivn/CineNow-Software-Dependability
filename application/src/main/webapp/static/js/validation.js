document.addEventListener('DOMContentLoaded', function () {
    function validateForm() {
        ...
        // If the expected elements are not present on the page, skip client validation
        if (!emailField || !passwordField || !errorMessage) {
            return true;
        }
    ...
    function isValidEmail(email) {
        // Regex per validare un'email in modo semplice ma efficace
    ...
    function validateFormReg() {
        ...
        // Se mancano elementi, non tentare la validazione client-side
        if (!nomeField || !cognomeField || !emailField || !passwordField || !confirmPasswordField) {
            return true;
        }
    ...
    // Aggiunge evento per rimuovere errori quando l'utente digita
    document.querySelectorAll('input').forEach(input => {
        if (input) {
    ...
    // Esporta le funzioni globalmente se necessario (per onsubmit in HTML)
    window.validateForm = validateForm;
    window.validateFormReg = validateFormReg;
});
